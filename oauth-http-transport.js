 ...
 */
module.exports = function( createRequest, oAuthCredentials ) {
	...
	function _auth( request, options, forceAuthentication ) {
		...
		// If no username or no password, try oauth
		if ( ! username || ! password ) {
			return _signOAuth(request);
		}
	...
	function _signOAuth( request, options ) {
	  ...
	  var oauth1 = new oauth.OAuth("", "", oAuthCredentials.clientId, oAuthCredentials.clientSecret, '1.0',null,'HMAC-SHA1' );
		var authHeader = oauth1.authHeader(request.url, oAuthCredentials.accessToken, oAuthCredentials.tokenSecret, request.method);
	...
	function paginateResponse( result, endpoint, httpTransport ) {
		...
		if ( links.next ) {
			result.body._paging.next = createRequest( mergeUrl( endpoint, links.next ) )

		}

		// Create a WPRequest instance pre-bound to the "prev" page, if available
		if ( links.prev ) {
			result.body._paging.prev = ncreateRequest( mergeUrl( endpoint, links.prev ) );
		}
